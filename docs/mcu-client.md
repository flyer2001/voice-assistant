...
## Статус проверки гарнитуры (2026-08-29)

**Гарнитура работает на рации — динамик и микрофон.** Это снимает подозрение
в браке (у первого экземпляра Vostok был 0 Ω, короткое в излучателе).

Подключение к телефону через покупной переходник TRS→TRRS **молчит**. Причина
не в гарнитуре и не в разделительном конденсаторе (он режет постоянку, звук
проходит), а в разводке: обычный аудиопереходник не знает про два раздельных
штекера K1 и раскладывает контакты по своему стандарту.

Разводка K1 ↔ TRRS (uhpowerup.com/k1-to-trrs):

| TRRS 3.5 | функция | сторона K1 |
|---|---|---|
| Tip | звук в наушник | большой штекер |
| Ring 1 | микрофон | 2.5 мм, Tip |
| Ring 2 | PTT | 2.5 мм, Ring |
| Sleeve | земля | общая обоих штекеров |

**Гипотеза «consumer-усилитель не потянет 10 Ω» пока не проверена** — до неё
дело не дошло. Проверяется переходником на две пайки: гнездо 3.5 (Tip, Sleeve)
на штекер TRS в выход мака. Если играет, MAX98357 нужен только автономному
девайсу, а для связки с телефоном или компьютером усилитель не требуется.

**PTT через пассивный переходник не заводится в принципе.** В гарнитуре земли
микрофона и динамика общие, кнопка просто замыкает пин на землю; в TRRS одна
земля на всё, различить нажатие нечем без активной электроники с питанием
(electronics.stackexchange 624092, 658131). На телефоне кнопка будет экранной.
Аппаратная PTT — единственное, что действительно требует MCU с GPIO.
...
⚠️ **Открытый вопрос — BTL-выход против общей земли.** Проверено по карточке
MH-ET LIVE MAX98357A (2026-08-29): выход мостовой, `OUTP`/`OUTN`, оба провода
активные, земли на выходе нет. А выше в этой же схеме `Sleeve` 2.5мм заявлен
общим с `Sleeve` 3.5мм — то есть минус динамика сидел бы на земле микрофона
и закоротил половину моста.

Радиостанция обходит это иначе: гонит динамик single-ended от половины своего
моста, отсюда и постоянка на линии (см. раздел выше).

Решать на этапе P2, до пайки корпуса. Первый шаг — прозвонить саму гарнитуру:
`Sleeve` 3.5 против `Sleeve` 2.5. Если звонятся — земли общие, и нужен либо
развязывающий трансформатор, либо single-ended усилитель с разделительным
конденсатором вместо прямого подключения BTL. Если не звонятся — BTL заходит
как есть.

Сам MAX98357A нужен в любом раскладе: он единственный в цепи принимает I2S.

Параметры платы: вход I2S 8-96 кГц (MCLK не нужен), питание 2.5-5.5 В,
3.2 Вт на 4 Ω / 1.8 Вт на 8 Ω, на 10 Ω ориентировочно 1.4 Вт — костной
проводимости хватает. Выводы `LRC` `BCLK` `DIN` `GAIN` `SD` `GND` `Vin`,
шаг 2.54, плата 17.7×19.1 мм, gain по умолчанию 9 dB, выход (L+R)/2.
Приходит без пайки — гребёнка и клеммник отдельно.
