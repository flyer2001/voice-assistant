## [2026-09-24] Дейлик и груминг: whisper умер незаметно, инцидент разобран до хука в парке

**Сделано:**
- ✅ Груминг по стокам и адресам записан целиком (288 блоков), выжимка + строка в INDEX
- ✅ Дейлик восстановлен из файла задним числом (37 блоков) — во время эфира трансляции не было, запись на маке уцелела
- ✅ Найдена причина пустых конспектов: idle-suspend на ubuntu-home убивает CUDA-контекст, `/health` при этом продолжает отдавать 200, а `/transcribe` — 500. Дальше whisper ушёл в crashloop (25 рестартов), модель не грузилась
- ✅ Лечение в моменте: `rmmod/modprobe nvidia_uvm` + рестарт whisper; сон глушился `mask` на время эфира и снят после
- ✅ `live_tail.sh`: pre-flight прогоном через модель (падает с инструкцией при не-200) + лог развёл `тишина` и `ОШИБКА whisper HTTP <код>` — коммит 42f6141
- ✅ `~/.claude/docs/live-transcribe.md` переписан: диагностика в три шага (звук → whisper → хвост), раздел про CUDA, `with-host.sh --hold` для долгих работ, снята усыпляющая формулировка «тишина в начале = норма»
- ✅ Две памятки в memory проекта: цепочка отказа CUDA и правило проверять текстом, а не процессом
- ✅ Инцидент передан в agentops — они нашли у себя неверный инвариант ADR-0005, поставили хук `agentops-cuda-resume` с ожиданием готовности GPU, закрыли тестом, раскатали правило «проверяй работу, а не доступность» в CLAUDE.md на ubuntu-home и mac-home
- ✅ Их хук проверен независимым кругом с нашей стороны: машина уснула сама, pre-flight поймал (HTTP 000), после подъёма модель отработала за секунды — круг записан в их ADR
- ✅ Задачи Сергея из обоих эфиров переданы в myRep и заведены там с датами (шесть штук, плюс схлопнут дубль с GNRL-85374)
- ✅ `bench/work/prompt.txt` — словарь под рабочие созвоны вместо конференционного

**Решения:**
- Watchdog хвоста **в deploy не заносим**: отказ редкий, лечится перезапуском, постоянная нянька в проде преждевременна. Рецепт лежит в доке, возвращаемся на втором таком дне. Зафиксировано у agentops как решение, а не недоделка
- Pre-flight проверяет **прогоном через модель**, а не запросом к `/health`: при убитой CUDA 500 приходит даже на тишину, то есть проверка ловит именно наш отказ. Тестовый wav собирается питоном — ffmpeg на ubuntu-home нет
- `mask` sleep-таргетов признан откатом, а не решением: отключает сон целиком и конфликтует с таймером простоя agentops. Заменён их хуком
- `systemd-inhibit` через ssh не годится вообще — умирает вместе с сессией

**Открытое:**
- На mac-home тот же класс отказов не проверен: там ollama с моделью на 24 ГБ, проверка «работой» загрузит её в память личной машины. agentops ждёт согласия Sergey
- Приватность остальных артефактов в `/srv/voice-out/` и политика по передаче рабочих созвонов наружу — висят с 23.09, техническими мерами не закрываются

**Файлы:**
- backend/voice-service/deploy/live-transcribe/live_tail.sh
- bench/work/prompt.txt
- ~/.claude/docs/live-transcribe.md
- ~/.claude/projects/-root-projects-voice/memory/reference_ubuntu_home_sleep_breaks_cuda.md
- ~/.claude/projects/-root-projects-voice/memory/feedback_verify_recording_actually_transcribes.md
- /srv/voice-private/live/2026-09-24 12-02-18{.md,.summary.md}
- /srv/voice-private/live/2026-09-24 12-45-45{.md,.summary.md}

## [2026-09-23] Дейлик + груминг вживую, iOS разблокирован, закрыта публичная раздача конспектов

**Сделано:**
- ✅ Живая транскрипция двух эфиров подряд (дейлик 12-06-19, груминг 12-47-39) — хвост подхватил обе записи без перезапуска
- ✅ Разбор груминга по ходу эфира: 4 попадания записаны в questions.md + тихие тексты в VK
- ✅ Выжимки написаны для обеих записей, INDEX.md обновлён двумя строками
- ✅ Итоги дейлика и груминга заинжекчены в avito-сессию (cmudp5yrfsz6qw20u3y2s2gf4)
- ✅ **Найдена и закрыта публичная раздача конспектов**: `/srv/voice-out` отдавался Caddy наружу с `file_server browse` и CORS `*` — все расшифровки рабочих созвонов за две недели читались по `https://cashflow-game.ru/voice-out/live/` без авторизации
- ✅ Конспекты перенесены в `/srv/voice-private/` (700, вне веб-рута), `browse` в Caddyfile выключен, TTS-раздача для mac-клиента сохранена (проверено запросами)
- ✅ Пути обновлены в трёх доках с объяснением, почему именно `voice-private`

**Решения:**
- Handle `/voice-out/*` не убивали целиком: оттуда mac-клиент качает TTS-mp3. Вместо этого вынесли приватное из веб-рута + сняли листинг. Удаление handle сломало бы voice-reply
- Хвост `live_tail.sh` менять не пришлось — каталог приходит аргументом `$2`, хардкода пути в скриптах нет (проверено grep'ом). Инжекты идут через Happy REST, с Caddy не связаны
- Транскрипты рабочих созвонов остаются вне гита — как и было. В репо только публичные доклады

**Открытое:**
- В `/srv/voice-out/` публично лежат `accuracy/*.ogg` (входящие голосовые из VK) и `mac-*.mp3` (TTS-ответы). Листинг закрыт, но имена предсказуемы по timestamp — не трогали, решение за Sergey
- Передача транскриптов рабочих созвонов в Anthropic API и содержательных кусков в VK — вопрос политики Авито по AI-инструментам, техническими мерами не закрывается
- Строки за 14-21 сентября в INDEX.md без описаний — записи не разбирались

**Файлы:**
- docs/live-transcribe.md
- backend/voice-service/deploy/live-transcribe/OPERATIONS.md
- /root/.claude/docs/live-transcribe.md (вне репо)
- /etc/caddy/Caddyfile (вне репо)
- /srv/voice-private/live/ (перенос из /srv/voice-out/live/)

## [2026-09-22] Живая транскрипция дейлика и груминга + архив расшифровок

**Сделано:**
- ✅ Подняли `live_tail.sh` на mac-work, приняли два эфира подряд: дейлик (~23 мин) и груминг (~37 мин)
- ✅ Сводка дейлика + разбор «что касалось Сергея» отправлены cross-session в myRep
- ✅ Задачи с груминга по Сергею отправлены в myRep отдельным инжектом, горящее вынесено первым блоком
- ✅ Собран черновик задачи «блок опт-подачи на iOS» к грумингу 23.09 — `/srv/voice-out/live/task-draft-ios-block.md`
- ✅ Action items по ходу эфира писались в `/srv/voice-out/live/questions.md`
- ✅ Заведён `/srv/voice-out/live/INDEX.md` — таблица всех 18 живых записей с 14.09
- ✅ Выжимки сегодняшних эфиров положены рядом с конспектами как `<имя>.summary.md`
- ✅ В `~/.claude/docs/live-transcribe.md` дописано правило: заполнять строку индекса при разборе записи

**Решения:**
- Архив живых транскрипций остаётся плоско в `/srv/voice-out/live/`, без переездов — конвенция `<имя>.md` + `<имя>.summary.md` + `INDEX.md` рядом
- Старые записи в индекс внесены строкой без описания: индексируем on-touch, не bulk-backfill'ом (user-scope правило docs hygiene)
- Ничего из этого не под гитом — рабочие созвоны

**Открытое:**
- Вечером 22.09 — захват вебинара по инвестициям и ИИ, понадобится свой `VOICE_PROMPT_FILE` с терминами темы
- Описания старых записей в `INDEX.md` пустые, заполнятся при разборе

**Файлы:**
- /srv/voice-out/live/INDEX.md
- /srv/voice-out/live/task-draft-ios-block.md
- /srv/voice-out/live/2026-09-22 12-05-05.summary.md
- /srv/voice-out/live/2026-09-22 12-46-52.summary.md
- ~/.claude/docs/live-transcribe.md

## [2026-09-21] Живая транскрипция двух рабочих созвонов — планирование S7 и 1:1

**Сделано:**
- ✅ Подняли ubuntu-home из выключенного состояния: WoL → загрузился Windows →
  ребут в Ubuntu по рецепту из `~/.claude/docs/live-transcribe.md`, whisper 200
- ✅ Хвост `live_tail.sh` отработал два эфира подряд (планирование спринта ~1 час,
  затем 1:1 ~11 мин), оба ушли в отдельные конспекты — файлы не смешались
- ✅ Режим молчаливого слушателя выдержан: за весь эфир ни одного голосового
  вмешательства, реплики только по прямому запросу Sergey'я текстом
- ✅ Сводка по планированию S7 отправлена cross-session в myRep
  (sid cmtjzd60st36gzc0uu2til4eu, status 200)
- ✅ Экшены и риски Sergey'я записаны в `/srv/voice-out/live/questions.md`
  отдельной секцией со ссылкой на полный конспект
- ✅ Конспект 1:1 удалён по просьбе Sergey'я (чужие карьерные планы),
  `tail.log` проверен на утечку текста — чисто

**Решения:**
- Промпт-файл под тему созвона не ставили: запись уже шла, а перезапуск посреди
  эфира теряет недоинжекченный буфер и гонит распознавание с нулевой позиции
- Сводку 1:1 cross-session не слали — личный разговор, не рабочая повестка.
  Правило «рабочие созвоны → сводка в профильную сессию» на 1:1 не распространяется

**Открытое:**
- Ссылка на конспект планирования живёт только в `questions.md` на VDS, вне гита —
  при чистке `/srv/voice-out/live/` потеряется. Sergey решил не дублировать в memory
- Блоки 1:1 остались в JSONL этой сессии; удаление конспекта их оттуда не убирает

**Файлы:**
- /srv/voice-out/live/questions.md (не под гитом)
- /srv/voice-out/live/2026-09-21 12-05-18.md (не под гитом)
## [2026-09-20] Podlodka iOS AI Crew разобран целиком + первые замеры по HYP-049

**Сделано:**
- ✅ Транскрибированы и разобраны четыре доклада сезона: Кодрян, Новичков, Быстров (живой конспект с эфира), Магда, Рубанов. Все транскрипты в `bench/podlodka/transcripts/`
- ✅ Собраны заметки в myRep по каждому докладу + транскрипты рядом, слоты в индексе сезона
- ✅ Извлечены презентации: Ёлчев, Быстров, Гришутин, Новичков, Магда, Рубанов — со сверкой «слайды против речи» по каждому
- ✅ Разобраны репозитории спикеров: `ios-harness` и `simboost` (Ёлчев), три репозитория Новичкова включая официальные скиллы Apple, `agent-workspace` (Кодрян), `mobile-sdd-factory` (Быстров)
- ✅ Сводная заметка сезона как точка входа + разложение по девяти гипотезам
- ✅ Заведена HYP-054 «визуальный оракул» с лестницей из шести уровней и фальсифицируемым экспериментом
- ✅ Записан Enterprise Context — пятый и единственный механизм против протухания доки с машинной сверкой
- ✅ План проверки гипотез `_project-hub/docs/verification-plan-2026-09.md`: 13 замеров с порогами, объявленными заранее, по четырём осям (автономия, тестирование, метрики, мутационная история)
- ✅ Прогнаны два первых эксперимента в agentops: e3 выполнен, e2 признан негодным (см. Решения)
- ✅ Два рабочих созвона (дейлик, передача контекста по скидкам) собраны и отправлены cross-session в `avito-aider-claude-cli`
- ✅ Рунбук живой транскрипции `~/.claude/docs/live-transcribe.md` + строка в индексе глобального CLAUDE.md (версия 5.0.0 → 5.1.0)
- ✅ Ревизия индекса гипотез: коллизия номеров разведена (рунбук → HYP-055), HYP-024 убрана из wip, мёртвые пути в доках починены, битая ссылка на несуществующую заметку снята
- ✅ Склеено и сконвертировано видео двух встреч Телемоста для выкладки

**Решения:**
- **e3 (асимметрия локализации) выполнен, исходное опасение снято.** Локализация 40%, проверка названного факта 90%, асимметрия Tyen воспроизвелась. Но вывод обратный ожиданию: канарейка работает в режиме проверки, а не поиска, то есть обходит слабое место по конструкции. Вердикты перечитывать не надо; граница названа числом
- **e2 (права на правку) прогнан и признан негодным.** Отнять Write и Edit не значит сделать read-only: Bash остался и пишет, 4 обхода из 10 по контрольным суммам. Наш механизм оказался таким же добровольным, как промпт у Быстрова, за что мы его критиковали. Переигровка с read-only монтированием, изоляцией стенда и третьим плечом
- **Главный побочный результат обоих замеров:** есть дефекты, не видные из артефакта — улики лежат снаружи. Три независимых пропуска на одном кейсе. Следствие: верификатору нужен не только предмет, но и внешние факты о среде
- **Чистые контроли брать из истории, а не изготавливать** — самодельные загрязняются настоящими дефектами, поймано на своём же стенде
- Доклад Крупенько (про кэш) полностью не разбирали осознанно: взяли два приёма, заметку не заводили
- Живой хвост транскрипции запускается без tmux — tmux-сервер падает при нехватке памяти и уносит его с собой

**Открытое:**
- Переигровка e2 с тремя условиями: изолированный стенд, read-only на уровне файловой системы с сохранённым Bash, третье плечо с внешними фактами о среде
- Аудит режима формулировок канареечных кейсов — поставлен первым: кейс в режиме поиска работает на 40%, и мы этого не знали
- Три величины базовой линии — единственный пункт, цена которого растёт с каждым днём промедления
- Список сторожей как знаменатель: «двадцать сторожей» нигде не зафиксировано, грепом находится 17 скриптов плюс другие имена
- Коллизия номеров HYP-044 (заведена приоритетной задачей в хабе)
- Гипотеза «принесённое знание против сужения поиска» — переворот объяснения e3, требует проверки

**Файлы:**
- bench/podlodka/transcripts/{kodryan,novichkov,ubeto,magda,rubanov}.md
- ~/.claude/docs/live-transcribe.md, ~/.claude/CLAUDE.md
- myRep: десять заметок сезона, сводка, Enterprise Context, девять гипотез, verification-plan-2026-09.md

## [2026-09-16] Подлодка: живая транскрипция в бою 3 дня, презентации и записи докладов

**Сделано:**
- ✅ Отработаны все записи 14-16.09: доклады Бугра, Мирзояна, Опарина, Гришутина плюс рабочие дейлики и груминги Sergey. Конспекты с таймкодами, попадания листов ожидания в `questions.md` (9 границ контекста)
- ✅ Заполнены заметки myRep по четырём докладам; заметки Опарина и Кодряна не существовало — созданы
- ✅ Разобраны четыре презентации субагентами (Бондаренко 86 слайдов, Опарин 43, Бугор 42, Кодрян 12), тексты дописаны в заметки, схемы сохранены картинками
- ✅ Скачаны записи Бондаренко и Кодряна в `/srv/voice-out/talks/` (mp3 + mp4 720p, 298 МБ)
- ✅ Заведены HYP-052 (состояние задачи вне контекста) и HYP-053 (compile-time рельсы), дополнены HYP-022 и HYP-037
- ✅ Управление живой транскрипцией принято от voice-73: tmux `live-tail`, скриншоты по таймкодам, журнал processed.md
- ✅ Рабочие созвоны Sergey разобраны и отправлены cross-session в avito-aider-claude-cli

**Решения:**
- YouTube не был заблокирован — apt-версия `yt-dlp` (2024.04.09) отдавала 403 на медиа. Свежая в venv качает с VDS без куки и прокси. Прежняя запись в CHANGELOG «YouTube blocked» была неверным диагнозом
- Голосовые сигналы в эфир отменены по фидбеку Sergey 14.09: попадания идут в questions.md и тихим текстом в VK
- Презентации разбирать субагентами, а не в основной сессии — экономит контекст на порядок

**Открытое:**
- Транскрибация обеих записей не начата — первый шаг следующей сессии
- precision/recall судьи у Бондаренко не измерены нигде, `evals` в оркестраторе есть, содержимое не показано — вопрос @lvbond, пока спикеры на связи
- У Кодряна нет ни одной метрики: обещанное сравнение «с рабочим пространством и без» на слайдах отсутствует
- mac-home оказался M1 Max 32 ГБ, а не 64 — «оптимальный старт» по шкале Опарина, не sweet spot
- Пилот по HYP-049 (чтение сырого reasoning локальной модели для аудита harness) не начат

**Файлы:**
- .claude/handoff/2026-09-16-podlodka-talks.md
- backend/voice-service/deploy/live-transcribe/live_tail.sh
- bench/podlodka/2026-09/transcripts/2026-09-15-grishutin.md
- bench/podlodka/2026-09/shots/processed.md
- myRep: заметки четырёх докладов, сводная, HYP-022/037/052/053

## [2026-09-16] Запись в дороге: mac-work достаточен, mac-home разобран впрок

**Сделано:**
- ✅ Проверен mac-home: ssh жив, macOS 26.6.2, OBS/ffmpeg/autossh/brew на месте — ставить нечего
- ✅ Включён `caffeinate -dims -t 21600` на mac-home (4 assertions, клэмшелл не усыпит)
- ✅ Алиас `mac-home-tun` (localhost:2222) возвращён в `/root/.ssh/config` для поездок
- ✅ Разобран сценарий записи вне дома, записан в OPERATIONS.md §«Запись в дороге»
- ✅ Распознаны два файла из Загрузок mac-work (2.5 с клип и 48-секундное голосовое про прокол)

**Решения:**
- **mac-work остаётся рабочей машиной записи даже в дороге** — обратный туннель исходящий (`-R 2223`), мобильный интернет ему безразличен. Исходная посылка «mac-work будет недоступен» неверна
- **BlackHole на mac-home не нужен** для записи звонков: OBS снимает системный звук через ScreenCaptureKit
- **VNC не носит звук** — слушать созвон на удалённом маке через Screen Sharing нельзя, OBS ставится там, где звук реально играет
- Живой хвост — best-effort: запись всегда лежит локально на маке, при рваной связи звук забирается после (opus 24k, ~11 МБ/час)

**Открытое:**
- Запись на mac-home «по требованию» не поднята — не понадобилось, лекция прослушана вживую. Остаётся GUI-настройка OBS + autossh с мака
- Диаризация для созвонов (с 15.09) — без движения

**Файлы:**
- backend/voice-service/deploy/live-transcribe/OPERATIONS.md
- .claude/TASKS.md
- /root/.ssh/config (вне репо)

## [2026-09-15] Harness живой транскрипции: от спайка до боевых докладов Podlodka

**Сделано:**
- ✅ Live-конвейер целиком: `live_tail.sh` (дежурный цикл, чанки 15с, блоки 30с инжектами в сессию-слушателя), `transcribe_file.sh` (готовые записи, 5× реального времени), `attach_screenshots.py` (склейка по таймкодам + журнал processed.md), OPERATIONS.md
- ✅ Первый день конференции в бою: 3 записи (Бугор 1:02, дейлик, Мирзоян 1:12), 15 попаданий листа ожидания на Бугре (baseline некорректен — подтверждено, авто-апрув 4% PR, ревью 3-5 дней), 16 скриншотов вклеено
- ✅ Роли: оркестратор (эта сессия) + слушатель (отдельная, молчаливый приём, questions.md) + myRep (листы ожидания из гипотез). Управление передано слушателю, хвост в tmux `live-tail`
- ✅ Прошлая Подлодка: 3 доклада скачаны, расшифрованы, разобраны субагентами на haiku до сводки (SUMMARY.md) — три спикера независимо сошлись на деградации длинного контекста
- ✅ Навык Алисы: маршрут /v1/alice/<секрет>, 17 тестов, задеплоен, сквозной прогон — второй канал ввода для коротких команд (монолог невозможен: у API нет управления микрофоном)
- ✅ Плеер ответов на mac-work/mac-home (LaunchAgent + voice-say/voice-say-target), DNS-кэш против сбоев резолвера
- ✅ Ресёрчи: Apple Watch Ultra 3 (не покупать до анонса Ultra 4), корзина MCU-железа (2.5мм разъёмы mono → нужны stereo, MAX98357A подтверждён, BTL vs общая земля K1), гарнитура жива — дело в разводке переходника
- ✅ Whisper accuracy: словарь influence > выбор модели, но эффект модель-зависим; на turbo промпт «примагничивает» имена проектов — выключен по умолчанию

**Решения:**
- Голосовые врывания в эфир запрещены (фидбек Sergey) — попадания в questions.md + тихий VK; в память проекта
- Запись руками кнопкой OBS, websocket/capture-obs не делаем (YAGNI)
- NDA снят как блокер (14.09): ограничитель — персональные данные
- Хвост намеренно тупой: сглаживание/анализ — в сессии, не в скрипте
- Скриншоты через хоткей OBS (⌘⇧Z): системный ⌘⇧4 перехвачен Яндекс.Диском

**Открытое:**
- Диаризация для созвонов (окно приложения / API Zoom-Толк / pyannote) — новая задача
- Улучшения по фидбеку: счётчик снимков, questions.md на запись, автоопределение доклада
- Sergey: навык Алисы в консоли, разметка WER-корпуса, pmset, BlackHole на mac-home
- 43 непушнутых коммита — push по команде Sergey

**Файлы:**
- backend/voice-service/deploy/live-transcribe/* (весь конвейер + OPERATIONS.md)
- backend/voice-service/deploy/vds-wrappers/* (voice-say, тесты)
- backend/voice-service/Sources/VoiceServiceCore/{AliceModels,Configuration,VoiceServiceApp,BearerAuthMiddleware}.swift
- bench/podlodka/* (транскрипты, сводка, листы ожидания, расписание)
- docs/{live-transcribe,alice-skill-input,where-runs-what}.md
