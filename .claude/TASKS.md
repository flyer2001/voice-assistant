...
Идея Sergey. Ресёрч показал: работает, вечер работы, всё нужное уже есть —
HTTPS-домен, Caddy, аккаунт Яндекса. Детали и ограничения:
[`docs/alice-skill-input.md`](../docs/alice-skill-input.md).

- [ ] Маршрут `/v1/alice` в voice-backend: разбор запроса Диалогов, инжект
      текста в сессию по фокусу, мгновенный ответ-заглушка
- [ ] Зарегистрировать приватный навык в консоли, привязать webhook
- [ ] Живая проверка с колонки, подобрать активационное имя (Алиса не должна
      путать его с другими)
