...
- [ ] **Добавить `initial_prompt` в whisper-эндпоинт на ubuntu-home** —
      у `/transcribe` только `audio` + `lang_hint`, промпт не принимается.
      Весь VK-ввод идёт без техлексики, отсюда `Веронись к диспетчеру` и
      `аудиосообщений 6`. На mac-пути промпт подаётся и там таких ошибок нет.
      Прогон 2026-08-01 показал: промпт влияет сильнее выбора модели
      ([comparison](../bench/whisper-accuracy/comparison-2026-08-01.md)).
      Done: эндпоинт принимает prompt, бот его шлёт, повторный прогон корпуса
