...
- [ ] **Решить, где живёт bws-токен вне VDS** — последний незакрытый пункт
      приёмки офсайт-бэкапа (2026-09-25). Фраза расшифровки архивов лежит
      в bws, а bootstrap-токен к bws — в `/root/.bitwarden/access.env` на VDS.
      Если больше нигде, пароль от бэкапа умирает вместе с бэкапируемым
      хостом. Ответ нужен от Sergey, дальше agentops впишет процедуру
      в `agentops/docs/disaster-recovery.md`. Всё остальное по приёмке
      закрыто: восстановление доказано побайтно с WebDAV, офсайт суточный,
      три ложных зелёных в скрипте исправлены, тесты 11 + 33
...
- [ ] **Приватность остальных артефактов в `/srv/voice-out/`** (23.09):
      конспекты вынесены в `/srv/voice-private/`, но публично остаются
      `accuracy/*.ogg` (входящие войсы из VK) и `mac-*.mp3` (TTS-ответы).
      Листинг снят, но имена предсказуемы по timestamp. Решить: закрывать
      или считать нечувствительным. Контекст —
      [live-transcribe.md §«Что где оседает»](../docs/live-transcribe.md)
- [ ] **Политика по передаче рабочих созвонов наружу** (23.09): текст
      уходит в Anthropic API, куски — в VK. Техническими мерами не
      закрывается, нужен ответ от Sergey / ИБ Авито, допустимо ли это
      по NDA. До ответа — держать содержательные куски в VK минимальными
