...
### S2 (Forward to Happy + bubble UI) — CLOSED 2026-06-18
...
- [x] **E2.4** (2026-06-14): `IntentPipeline` (`Sources/VoiceAssistant/Backend/IntentPipeline.swift`) glues transcript → BackendAdapter.send → TurnsStore.append + updateReply. Appends a pending turn before awaiting so query bubble shows immediately. Error labeling (`label(for: BackendError)`) lives in pipeline, shared across future clients. ContentView refactored: TurnsStore @State + ScrollView/LazyVStack of TurnView + autoscroll to newest; STT errors stay in `lastError` footer line, backend errors land in turns as `.failure(label)`. 5 IntentPipelineTests (FakeBackendAdapter fixture). Full SPM suite **36/36 green** + iOS **BUILD SUCCEEDED** + installed/launched on iPhone 17 sim (com.voiceassistant.app.flyer2001 pid 52661).
- [x] **E2.5** (2026-06-17): `TokenStore` protocol + `InMemoryTokenStore` (NSLock-guarded, init(initial:) seed for tests) + `KeychainTokenStore` (kSecClassGenericPassword via SecItemAdd/Update/CopyMatching/Delete; idempotent write through update-then-add fallback). `KeychainTokenStoreError` surfaces OSStatus + .malformed. `OnboardingView` sheet (NavigationStack + monospaced TextField + interactiveDismissDisabled when empty) shown via `.task` on launch when read() returns nil. ContentView: hold-button + F15 disabled until token present, header key-icon reopens sheet for rotation, LiveSTTUploader + DispatcherAdapter constructed inline per request with current token (no stale-token bugs after save). 5 InMemoryTokenStoreTests green (Keychain integration tested manually in sim per TESTING.md §"Mock только boundaries"). Full SPM suite **41/41 green** + iOS **BUILD SUCCEEDED** on mac-work (mac-home unreachable mid-session — IP shifted in ssh config, separately tracked). Onboarding sheet verified in iPhone 17 sim with screenshot.
- [x] **E2.6** (2026-06-18): E2E smoke through real stack via curl from mac-home loopback. Stack came together: Whisper FastAPI **relocated to ubuntu-home** (dual-boot ОС, RTX 3070 CUDA, faster-whisper-large-v3-turbo) — `pip install nvidia-cublas-cu12 nvidia-cudnn-cu12` + `LD_LIBRARY_PATH` setup. Backend `voice-service` on mac-home `STT_MODE=live WHISPER_URL=http://192.168.88.13:8000` restarted (prior instance wedged after 2.5 days + mac sleep cycles). Sample `assets/bench/raw/1a-quiet.m4a` (14.6s RU). **S1 path** POST `/v1/voice/audio`: 200 OK in 640 ms, transcript "Сегодня вторник, нужно успеть в магазин, пить молоко, хлеб, помидоры и заехать на заправку." (stt_ms=543 на CUDA). **S2 path** POST `/v1/voice/intent`: 200 OK in 8 ms, reply `[live reply] <transcript>` (backend stubs Happy inject as echo until real session bind — separate B-series). Wire contract honored end-to-end. **Story S2 CLOSED**.
